(function () {
    ...
    function appendTable(data) {
        ...
        data_districts.find('tbody').append('\
                        <tr id="filter-form-' + data.id + '" data-src="' + src + '">\
                        <td>' + data.title + '</td>\
    ...
    $(document).delegate('.icon_edit', 'click', function () {
        ...
        images.forEach(function (p1, p2, p3) {
            if (p1 !== '') {
                $('#images').append('<li><img src="/oc-content/uploads/repair/preview_' + p1 + '" class="img-responsive"><i data-name="' + p1 + '" class="glyphicon glyphicon-remove text-danger image-name"></i></li>');
            }
        });
    ...
    $('.add-photo').on('click', function (e) {
        $(this).prev('div')
            .append('<div><input type="file" name="photos[]" accept="image/jpeg"></div>');
