<?php
    ...
    class ModelRealEstate extends DAO
    {
        ...
        {
            $item = Item::newInstance()->findByPrimaryKey($array['itemId']);
            $aSet = array(
        ...
        public function replaceAttr($array)
        {
            ...
            $item = $this->getAttributes($array['itemId']);

            if($item['fk_i_item_id']){
                return  $this->dao->update( $this->getTable_HouseAttr(), $aSet, ['fk_i_item_id'=> $array['itemId']]);
            }else{
                return $this->dao->insert( $this->getTable_HouseAttr(), $aSet);
            };
        } 
