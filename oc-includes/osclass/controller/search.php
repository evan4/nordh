 ...
 */
    ...
    class CWebSearch extends BaseModel
    {
        ...
        function doModel()
        {
            ...
            $p_floor    = Params::getParam('floor');
            if(!is_array($p_floor)) {
                if($p_floor == '') {
                    $p_floor = '';
                } else {
                    $p_floor = explode(",", $p_floor);
                }
            }


            $p_rooms     = [];
            $p_rooms[] =  (Params::getParam('rooms-0') == 121) ? '0' : '';
            $p_rooms[] =  Params::getParam('rooms-1');
            $p_rooms[] =  Params::getParam('rooms-2');
            $p_rooms[] =  Params::getParam('rooms-3');
            $p_rooms[] =  Params::getParam('rooms-4');
            $p_rooms[] =  Params::getParam('rooms-5');
            $p_rooms[] =  Params::getParam('rooms-6');
            $p_rooms[] =  Params::getParam('rooms-6');
            $p_rooms[] =  Params::getParam('rooms-7');
            $p_rooms[] =  Params::getParam('rooms-8');
            $p_rooms[] =  Params::getParam('rooms-9');
            $p_rooms[] =  Params::getParam('rooms-10');
            $p_rooms = array_unique(array_diff($p_rooms, array(NULL)));

            $p_areaMin     = Params::getParam('areaMin');
            $p_areaMax    = Params::getParam('areaMax');
            ...
            //FILTERING BY FLOOR
            if($p_areaMin != '' || $p_areaMax != '') {
                $this->mSearch->areaRange($p_areaMin, $p_areaMax);
            }

            if(Params::getParam('address') != ''){
                $this->mSearch->addAddress(str_replace(',',' ',trim(strtolower(Params::getParam('address')))));
            }

            $floorsMin = Params::getParam('floorsMin');
            $floorsMax = Params::getParam('floorsMax');

            if($floorsMin != '') {
                if($floorsMax != '') {
                    $this->mSearch->floorsRange($floorsMin, $floorsMax);
                }else{
                    $this->mSearch->floorsRange($floorsMin, '');
                }
            }elseif ($floorsMax != ''){
                $this->mSearch->floorsRange('', $floorsMax);
            }

            if($floorMin != '') {
                if($floorMax != '') {
                    $this->mSearch->floorRange($floorMin, $floorMax);
                }else{
                    $this->mSearch->floorRange($floorMin, '');
                }
            }elseif ($floorMax != ''){
                $this->mSearch->floorRange('', $floorMax);
            }

            if(!empty($p_rooms)) {
                $this->mSearch->addRooms($p_rooms);
            }
            if($p_User) {
                $this->mSearch->addUser($p_User);
            }
