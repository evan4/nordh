 ...
 */
    ...
    class Search extends DAO
    {
        ...
        public function addFloor($floor = array())
        {
            if(is_array($floor)) {
                foreach($floor as $c) {
                    $c = abs($c);
        ...
        public function floorsRange($floorsMin, $floorsMax)
        {
            if($floorsMin != ''){
                $this->floorsMin = (int)$floorsMin;
            }
            if($floorsMax != '') {
                $this->floorsMax = (int)$floorsMax;
            }
        }

        public function floorRange($floorMin, $floorMax)
        {
            if($floorMin != ''){
                $this->floorMin = (int)$floorMin;
            }
            if($floorMax != '') {
                $this->floorMax = (int)$floorMax;
            }
        }

        public function areaRange($areaMin, $areaMax)
        {
            $this->areaMin = (int)$areaMin;
            $this->areaMax = (int)$areaMax;
        }

        public function addRooms($rooms = array())
        {
            if(is_array($rooms)) {
                foreach($rooms as $c) {
                    $c = abs($c);
        ...
        private function _conditions()
        {
            ...
            if($this->floorMin>0 || $this->floorMax>0 || $this->floorsMin>0 || $this->floorsMax>0 || !empty($this->rooms) || $this->areaMin != '' || $this->areaMax != '') {
                $this->withAttrs= true;
            }
        ...
        private function _makeSQL($count = false,$premium = false)
        {
            ...
            if($this->withItemId) {
                ...
                $this->dao->from(DB_TABLE_PREFIX.'t_item');
                if($this->user != ''){
                    $this->dao->where(DB_TABLE_PREFIX."t_item.s_contact_name", $this->user);
                    $this->dao->groupBy(DB_TABLE_PREFIX.'t_item.pk_i_id');
                }
                ...
                if($this->address !=''){
                    ...
                    $this->dao->where(sprintf("MATCH(%st_item_location.s_address) AGAINST('*%s*' IN BOOLEAN MODE)",DB_TABLE_PREFIX, $this->address));
                ...
                if($this->withAttrs){
                    ...
                    $this->dao->join(sprintf('%st_item_house_attr', DB_TABLE_PREFIX), sprintf('%st_item_house_attr.fk_i_item_id = %st_item.pk_i_id', DB_TABLE_PREFIX, DB_TABLE_PREFIX), 'LEFT');

                    if($this->areaMin > 0 ){
                        $this->dao->where(sprintf("%st_item_house_attr.s_square_meters >= %d", DB_TABLE_PREFIX, $this->areaMin));
                    }
                    if ($this->areaMax > 0 && $this->areaMax >=$this->areaMin){
                        $this->dao->where(sprintf("%st_item_house_attr.s_square_meters <= %d", DB_TABLE_PREFIX, $this->areaMax));
                    }

                    if($this->floorMin > 0){
                        $this->dao->where(sprintf("%st_item_house_attr.i_floor_number >= %d ", DB_TABLE_PREFIX, $this->floorMin));
                    }
                    if($this->floorMax > 0){
                        $this->dao->where(sprintf("%st_item_house_attr.i_floor_number <= %d ", DB_TABLE_PREFIX, $this->floorMax));
                    }
                    if($this->floorsMin > 0){
                        $this->dao->where(sprintf("%st_item_house_attr.i_num_floors >= %d ", DB_TABLE_PREFIX, $this->floorsMin));
                    }
                    if($this->floorsMax > 0){
                        $this->dao->where(sprintf("%st_item_house_attr.i_num_floors <= %d ", DB_TABLE_PREFIX, $this->floorsMax));
                    }
                    if(!empty($this->rooms)){

                        $this->dao->where(sprintf("%st_item_house_attr.i_num_rooms", DB_TABLE_PREFIX) .' IN ('. implode(', ', $this->rooms) .')' );
                    }
        ...
        /**
         * Return total items on t_item without any filter_search
         *
         * @return type
         */
        ...
        /**
         * Return latest posted items, you can filter_search by category and specify the
